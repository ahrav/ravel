//! RVL2 uses a separate disposable file from frozen RVL1.
//! Version 1 persists only unowned epoch-1, plan-stable histories.
//! First epoch or plan transition must bump or relax this schema.
...
use crate::{
    domain::campaign::ValidationError,
    scoped::{Digest, EventEnvelope, ScopeEventRef, ScopeHead, ScopeIdentity},
};

const APPLICATION_ID: i32 = 0x5256_4c32;
...
impl fmt::Display for ValidateError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(match self {
            ...
            Self::WrongSchemaVersion => "scope database schema version does not match",
            Self::IntegrityCheckFailed => "scope database integrity check failed",
    ...
    transaction.execute(
        "INSERT INTO applied_scope_events \
         (scope_id, sequence, digest, parent_digest, operation_id, writer_epoch, \
          payload_type, payload_version) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)",
        params![
            scope_id,
            stored_sequence,
            event.reference.digest().as_str(),
            ...
            i64::try_from(event.envelope.payload_version())
                .map_err(|_| ApplyError::DatabaseOperationFailed)?,
        ],
    )?;
    transaction.commit()?;
...
mod tests {
    ...
    use crate::{
        db::schema,
        distributed::presence::WorkspaceId,
        scoped::{CampaignId, EventEnvelope, ScopeEventRef, ScopeIdentity},
    };
    ...
    #[test]
    fn creates_distinct_rvl2_schema_and_reopens_it() {
        ...
        let v1 = path("v1");
        drop(schema::create(&v1).unwrap());
        assert_eq!(
            open_existing(&v1).unwrap_err(),
    ...
    fn applies_idempotently_and_rejects_gaps_without_mutation() {
        ...
        assert!(
            scope_matches_head(
                ...
                &ScopeHead::new(
                    scope.clone(),
                    crate::scoped::ScopeAuthority::Unowned,
                    1,
    ...
    fn validation_rejects_orphaned_history() {
        ...
        connection
            .execute(
                "INSERT INTO applied_scope_events \
                 (scope_id, sequence, digest, parent_digest, operation_id, writer_epoch, payload_type, payload_version) \
                 VALUES (?1, 1, ?2, NULL, 'operation', 1, 'root_genesis', 1)",
