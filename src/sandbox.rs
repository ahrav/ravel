...
use crate::{
    ...
    materialize::TRUSTED_CARGO_CONFIG,
    scope::Digest,
    storage::s3::S3Store,
};
...
mod contain {
    ...
    use super::{OVERLAY_BYTES, TMPFS_MAGIC, TRUSTED_CARGO_CONFIG, UnsupportedHost};
    ...
    pub fn build_fixture(
        ...
        match kind {
            ...
            FixtureKind::Evaluator => {
                ...
                write("pf.c", b"int main(void) { return 0; }\n")?;
                write(
                    "Cargo.toml",
                ...
                write(".cargo/config.toml", TRUSTED_CARGO_CONFIG)?;
                write("build.rs", b"fn main() {}\n")?;
                write(
                    "src/lib.rs",
                    b"pub fn exit_success() -> libc::c_int {\n    libc::EXIT_SUCCESS\n}\n\n#[test]\nfn evaluator_passes() {\n    assert_eq!(exit_success(), 0);\n}\n",
                )?;
            }
...
pub fn preflight(
    ...
    let (root, source) = prepare_probe_root(roots, "preflight", contain::FixtureKind::Evaluator)?;
    ...
    for command in [
        ...
        EvaluatorCommand::Test,
        EvaluatorCommand::FmtCheck,
        EvaluatorCommand::Clippy,
    ] {
        let evaluated = launch("evaluator", command.argv(), OverlayOptions::Frozen)
            .map_err(|_| UnsupportedHost::LaunchFailed)?;
