//! Immutable scoped-v2 event reads and publication witnesses.
//!
...
use crate::{
    scoped::{
        ...
        ROOT_GENESIS_PAYLOAD_TYPE, ROOT_GENESIS_PAYLOAD_VERSION, RootEvent, ScopeEventRef,
        ScopeIdentity, decode_scope_event, encode_root_event, scope_event_key,
    },
...
/// ResolvedScopeEventPublication proves canonical event bytes exist in one object-store namespace.
...
pub(crate) fn payload_registered(envelope: &EventEnvelope) -> bool {
    if envelope.payload_type() == ROOT_GENESIS_PAYLOAD_TYPE
        && envelope.payload_version() == ROOT_GENESIS_PAYLOAD_VERSION
    {
        return true;
    }
    #[cfg(test)]
    {
        envelope.payload_type() == crate::scoped::TEST_SUCCESSOR_PAYLOAD_TYPE
...
fn validate_registered(
    ...
    if envelope.payload_type() == ROOT_GENESIS_PAYLOAD_TYPE {
        let root = crate::scoped::decode_root_event(encoded.stored_bytes(), key, scope)
    ...
    #[cfg(test)]
    if envelope.payload_type() == crate::scoped::TEST_SUCCESSOR_PAYLOAD_TYPE
        && envelope.payload_version() == 1
    {
...
mod tests {
    ...
    async fn encoded_bytes_must_match_the_supplied_envelope() {
        ...
        let envelope_a = EventEnvelope::new(
            ...
            1,
            "operation-a".into(),
            crate::scoped::TEST_SUCCESSOR_PAYLOAD_TYPE.into(),
            1,
        )
        .unwrap();
