...
use super::{
    WireError,
    scope_event::{
        ResolvedScopeEventPublication, ScopeEventPublicationError, publish_root, read_opaque,
    },
};
...
mod tests {
    ...
    use crate::{
        ...
        sync::scope_event::publish_encoded,
    };
    ...
    async fn published(
        ...
        encoded: crate::scoped::EncodedScopeEvent,
    ...
    fn successor(
        ...
        sequence: u64,
        operation: &str,
    ) -> (EventEnvelope, crate::scoped::EncodedScopeEvent) {
    ...
    async fn cross_namespace_witnesses_never_dispatch_head_mutations() {
        ...
        let publication = published(
            genesis.identity(),
            root.envelope(),
            crate::scoped::encode_root_event(&root).unwrap(),
        )
        .await;
        let transition = ScopeHeadTransition::new(
            ScopeHeadParent::Genesis,
