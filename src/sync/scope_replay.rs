//! Scope replay separates asynchronous object reads from blocking RVL2 transactions.
//!
...
use crate::{
    db::scope_projections::{self, ApplyError, ApplyOutcome, ScopeProjectionEvent},
    scoped::{Digest, ScopeAuthority, ScopeEventRef, ScopeIdentity, root_event_from_decoded},
    storage::s3::S3Store,
};

use super::{
    WireError,
    scope_event::{ScopeEventReadError, payload_registered, read_opaque},
    scope_head::{self, ObservedScopeHead, ScopeHeadReadError},
};
...
struct Prepared {
    decoded: crate::scoped::DecodedScopeEvent<Value>,
}
...
pub async fn refresh_blocking(
    ...
    let cursor = match scope_projections::scope_cursor(connection, scope) {
        Ok(cursor) => cursor,
...
pub fn open_projection(path: &Path) -> Result<rusqlite::Connection, ScopeReplayError> {
    ...
    {
        return scope_projections::create(path).map_err(|_| ScopeReplayError::DatabaseUnavailable);
    }
    match scope_projections::open_existing(path) {
        Ok(connection) => Ok(connection),
        Err(scope_projections::ValidateError::DatabaseOperationFailed) => {
...
fn rebuild_projection(path: &Path) -> Result<rusqlite::Connection, ScopeReplayError> {
    ...
    scope_projections::create(path).map_err(|_| ScopeReplayError::DatabaseUnavailable)
}
...
async fn prepare_suffix_with_limits(
    ...
    let observed = match scope_head::read(store, scope).await {
        Ok(Some(head)) => head,
...
pub(crate) fn apply_suffix(
    ...
    for event in &prepared.events {
        if scope_projections::scope_contains_operation(
            connection,
            scope,
    ...
    let mutations = prepared
        ...
        .map(|prepared| {
            ...
            let envelope = if prepared.decoded.envelope().payload_type()
                == crate::scoped::TEST_SUCCESSOR_PAYLOAD_TYPE
            {
    ...
    for mutation in mutations {
        match scope_projections::apply_scope_event(connection, &mutation) {
            Ok(ApplyOutcome::Applied | ApplyOutcome::AlreadyApplied) => {}
            Err(error) => return Err(ScopeReplayError::Apply(error)),
        }
    }
    if !scope_projections::scope_matches_head(connection, prepared.observed.head())
        .map_err(ScopeReplayError::Apply)?
    {
        return Err(ScopeReplayError::HistoryConflict);
    }
...
mod tests {
    ...
    use crate::{
        ...
        scoped::{
            ...
            ROOT_GENESIS_PAYLOAD_VERSION, ScopeAuthority, ScopeEventRef, ScopeHead,
            TEST_SUCCESSOR_PAYLOAD_TYPE, encode_head, encode_scope_event, root_genesis,
        },
    ...
    fn genesis() -> crate::scoped::RootGenesis {
        root_genesis(
    ...
    fn root_mutation(genesis: &crate::scoped::RootGenesis) -> ScopeProjectionEvent {
        let root = crate::scoped::decode_root_event(
            genesis.event_bytes(),
            genesis.event_key(),
            genesis.identity(),
        )
    ...
    async fn replay_is_idempotent_and_returns_the_observed_head_witness() {
        ...
        let mut connection = scope_projections::create(&path).unwrap();
        ...
        assert_eq!(
            scope_projections::scope_cursor(&connection, genesis.identity()).unwrap(),
    ...
    async fn cursor_conflicts_fail_closed_and_invalid_projection_rebuilds() {
        ...
        let mut connection = scope_projections::create(&path).unwrap();
        scope_projections::apply_scope_event(&mut connection, &root_mutation(&genesis)).unwrap();
        connection
            .execute(
    ...
    async fn database_failure_keeps_a_valid_committed_prefix_not_ready() {
        ...
        connection
            ...
            .unwrap();
        drop(connection);
        drop(scope_projections::open_existing(&path).unwrap());
        fs::remove_file(path).unwrap();
    }
